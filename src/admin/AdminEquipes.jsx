...
export default function AdminEquipes() {
  ...
  return (
    <div className="admin-equipes">
      ...
      console.error('Erro ao adicionar crafter na equipe:', error);
      setMessage('Erro ao adicionar crafter na equipe. Tente novamente.');
    }
  };

  // Função para associar mentor ao projeto
  const handleAssociarMentorProjeto = async (mentorId, projetoId) => {
    try {
      ...
      if (response.ok) {
        setMessage('Mentor associado ao projeto com sucesso!');
        notifyDataChange('mentors');
      } else {
        throw new Error('Erro na resposta do servidor');
      }
    } catch (error) {
      console.error('Erro ao associar mentor ao projeto:', error);
      setMessage('Erro ao associar mentor ao projeto. Tente novamente.');
    }
  };
        ...
        <div className="tab-content">
          <div className="section">
            ...
            {Object.keys(groupedTeams).length === 0 ? (
              ...
              Object.entries(groupedTeams).map(([projetoId, equipes]) => (
            ...
            <div className="form">
              <div className="form-row">
                <div className="form-group">
                  ...
                  <select
                    value={newTeam.projeto_id}
                    onChange={(e) => setNewTeam({...newTeam, projeto_id: e.target.value})}
                    required
                  >
                    <option value="">Selecione um Projeto</option>
                    {projects.map(projeto => (
                ...
                <div className="crafters-selection-improved">
                  ...
                  <div className="selection-summary">
                    <span className="selected-count">
                      ✅ {newTeam.crafters_ids.length} crafter(s) selecionado(s)
                    </span>
                  </div>
                </div>
              </div>
              ...
              <button 
                ...
                disabled={newTeam.crafters_ids.length === 0}
              >
                🚀 Criar Equipe ({newTeam.crafters_ids.length} crafter{newTeam.crafters_ids.length !== 1 ? 's' : ''})
              </button>
            </div>
          </div>
        </div>
