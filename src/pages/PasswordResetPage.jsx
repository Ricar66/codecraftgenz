...
import { resetAdminPassword } from '../services/userAPI.js';

export default function PasswordResetPage() {
  const [email, setEmail] = useState('admin@codecraft.local');
  ...
  const onSubmit = async (e) => {
    ...
    setSuccess('');
    setLoading(true);
    const res = await resetAdminPassword({ email, newPassword, token });
    setLoading(false);
    if (!res.ok) {
  ...
  return (
    <div className="reset-page">
      ...
      <section className="section-block">
        <div className="card">
          <h1 className="title">Redefinir Senha (Admin)</h1>
          <p className="subtitle">Use o token de reset fornecido pelo administrador.</p>
          ...
          <form className="form" onSubmit={onSubmit}>
            ...
            <input className="input" type="text" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Insira o token de reset" required />
      ...
      <style>{`
        .reset-page { min-height: 100vh; background-color: transparent; }
